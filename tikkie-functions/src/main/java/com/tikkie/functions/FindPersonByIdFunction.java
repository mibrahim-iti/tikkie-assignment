 ...
 */
public class FindPersonByIdFunction implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {

    private final PersonService personService;

    public FindPersonByIdFunction() {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent requestEvent, Context context) {
        ...
        try {
            ...
            pathParameters.entrySet().forEach(entry -> logger.log(entry.getKey() + " " + entry.getValue()));
            logger.log("pathParameters end");

            final Optional<Person> optionalPerson = personService.find(pathParameters.get("id"));
            if (optionalPerson.isPresent()) {
                return tikkieResponseUtil.returnOk(optionalPerson.get());
            }

            return tikkieResponseUtil.returnNotFound("The person you are looking for is not exist");
